...
function BalaoEsquerdo(props: BalaoMensagemProps) {
    return (
        <div className={`flex gap-4 ${props.omitirAutor && "pl-16"}`}>
            {!props.omitirAutor && <Image src={"/chat.svg"} alt={"Assistente"} width={40} height={40} />}
            <div className="flex flex-col">
                {!props.omitirAutor && <span className="text-xs text-zinc-600">{props.mensagem.autor}</span>}
